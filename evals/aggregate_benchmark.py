...
def _summary(records: Sequence[Mapping[str, Any]]) -> dict[str, dict[str, float]]:
    return {
        ...
        "duration_ms": calculate_stats(
            [float(record["result"]["duration_ms"]) for record in records]
        ),
        "time_seconds": calculate_stats(
            [float(record["result"]["time_seconds"]) for record in records]
        ),
...
def build_benchmark(
    ...
    *,
    ...
    by_config = {
        config: [run for run in runs if run["configuration"] == config] for config in CONFIGS
    }
